...
router.post("/insert", (request, responce) => {
  ...
  cryptPassword(request.body.password, (e, encrypted) => {
    let usersModelObject = new usersModel({
      ...
      petdiscription: request.body.petdiscription,
    })
    usersModelObject
      .save()
      .then((callbackData) => {
...
router.post("/update/:id", (request, responce) => {
  usersModel.findById(request.params.id, (error, userData) => {
    if (error) {
      ...
      if (request.body.petName != userData.petName) {
        toUpdate.petName = request.body.petName
      }
      if (request.body.petdiscription != userData.petdiscription) {
        toUpdate.petdiscription = request.body.petdiscription
      }
